...
def export_report(project_id: str, payload: ExportReportRequest) -> Response:
    ...
    for mat in all_materials:
        if 'sqft' in mat['unit'].lower():
            cost = total_area_m2 * 10.764 * mat['cost_per_unit']
        elif 'cum' in mat['unit'].lower() or 'm3' in mat['unit'].lower():
            cost = total_volume_m3 * mat['cost_per_unit']
        else:
            cost = total_volume_m3 * mat['cost_per_unit'] * 50
        material_costs.append({'name': mat['name'], 'cost': cost, 'strength': mat['strength'], 'durability': mat['durability']})
    ...
    for idx, mat in enumerate(material_costs):
        if idx == 0:
            pdf.set_fill_color(220, 252, 231)  # Green - cheapest
        elif idx == len(material_costs) - 1:
            pdf.set_fill_color(254, 226, 226)  # Red - most expensive
        else:
